...
router.post('/updataAdmin', (req, res) => {
    ...
    UserInfo.updateOne({ _id }, { $set: { isAdmin: !admin } }, (err) => {
        if (err) {
            if (admin) {
...
router.post('/createArticle', (req, res) => {
    const { title, author, categorys, description, content, contentHtml, isTop, isRecommend } = req.body;
    Article.findOne({
        title
    }).then((result) => {
        if (result) {
...
router.post('/editArticle', (req, res) => {
    const { title, author, categorys, description, content, contentHtml, isRecommend, isTop, _id } = req.body;
    Article.findOne({
        title
    }).then((result) => {
        if (result) {
        ...
        Article.updateOne(
            { _id },
            { $set: { title, author, categorys, description, content, contentHtml, isTop, isRecommend } },
            (err) => {
                if (err) {
